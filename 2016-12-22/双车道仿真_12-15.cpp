...
void Road::AddVehicle(CObList* &plist, const int &carSum, const double &Length, const double &carHeadway, const bool &lanenumber, double OptimumV, double distf) //distf
{
	for (int i = 1; i<carSum + 1; i++)
	{
		...
		{
			Vehicle* vehicle = new Vehicle(i,             Length - i*carHeadway        , OptimumV, 0, carHeadway, lanenumber, distf);  //加车易错
			plist->AddTail(vehicle);
		}
	}
}
...
///////////////////////////////////////////////////////////////////
	...
	while (pos)  //遍历
	{
		...
		//加入扰动，第25辆车与前车的车间距减至原始车间距的90%
		//车头距会有更新，所以多次设定
		if (i >= 100 && i <= 150)
		{
			if (thisCar->Number == 25)
			{
				thisCar->Headway = thisCar->Headway * 0.9;
				thisCar->Position[0] = thisCar->Position[0] + 0.1 * thisCar->Headway;
				thisCar->Distf[0] = thisCar->Distf[0] - 0.1 * thisCar->Headway;
				//判断是否超出
				if (thisCar->Position[0] >= L)
					thisCar->Position[0] = thisCar->Position[0] - L;
			}
		...
		{
			if (thisCar->LaneNumber == 0)
				outfile1 << i << "   " << thisCar->Number << "   " << thisCar->LaneNumber << "   " << thisCar->Position[tao] << "   " << thisCar->Speed[tao] << "   " << thisCar->Acceleration << "   " << thisCar->Headway << endl;
			else
				outfile2 << i << "   " << thisCar->Number << "   " << thisCar->LaneNumber << "   " << thisCar->Position[tao] << "   " << thisCar->Speed[tao] << "   " << thisCar->Acceleration << "   " << thisCar->Headway << endl;
		}
...
void Road::InsertAtMiddle(CObList* &porigin, CObList* &ptarget, Vehicle* &thisCar, Vehicle* &carLeading, int i)
{
	...
	if (!porigin->IsEmpty())
	{
		...
		//输出换道信息/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		if (thisCar->LaneNumber == 0)
			outfile3 << i << "   " << thisCar->Number << "   " << thisCar->LaneNumber << "   " << thisCar->Position[0] << "   " << thisCar->Speed[0] << "   " << thisCar->Acceleration << "   " << thisCar->Headway << endl;
		else
			outfile4 << i << "   " << thisCar->Number << "   " << thisCar->LaneNumber << "   " << thisCar->Position[0] << "   " << thisCar->Speed[0] << "   " << thisCar->Acceleration << "   " << thisCar->Headway << endl;
